...
SLOT_ORDER = ["name", "eatType", "food", "priceRange", "area", "near", "familyFriendly", "customerRating"]
...
def slot_signature(slots: dict[str, Any]) -> str:
    return "|".join(slot for slot in SLOT_ORDER if slot in slots)


def slots_to_features(slots: dict[str, Any], head: str) -> dict[str, Any]:
    fs: dict[str, Any] = {
        "HEAD": head,
        "slotset": slot_signature(slots),
    }
    ...
    with Path(args.items).open("r", encoding="utf-8") as f, manifest_path.open("w", encoding="utf-8") as out:
        for idx, line in enumerate(f, 1):
            if not line.strip():
                continue
            row = json.loads(line)
            item_id = row.get("fragment_id") or row.get("id") or f"e2e_test_{idx:05d}"
            slots = row["expected_slots"]
            signature = slot_signature(slots)
            fs = slots_to_features(slots, args.head)
            input_path = input_dir / f"{item_id}.input"
            input_path.write_text(
                args.symbol + " " + render_feature_structure(fs) + "\n",
                encoding="utf-8",
            )
            manifest = {
                "id": item_id,
